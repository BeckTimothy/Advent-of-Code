/**
 * This is a refactored version of challenge 1 of 2019-12-09 of Advent of Code's 25-day challenge
...
function intcodeProgram(arr, input) {
	...
	for(let i = 0; i <= arr.length - 1; i) {
		...
		let paramModeOne = Number(arr[i].toString().charAt(arr[i].toString().length - 3));
		//targets the parameter for opcodeParamTwo in the opcode address; two digits before address
		let paramModeTwo = Number(arr[i].toString().charAt(arr[i].toString().length - 4));
		//targets the parameter for opcodeParamThree in the opcode address; three digits before address
		let paramModeThree = Number(arr[i].toString().charAt(arr[i].toString().length - 5));

		//sets parameter mode of opcode parameter one
		if(paramModeOne === 1) {
			//immediate mode
			opcodeParamOne = arr[i + 1];
		} else if(paramModeOne === 2) {
			//relative mode
			opcodeParamOne = arr[relativeBase + arr[i + 1]];
		} else {
			//position mode
			opcodeParamOne = arr[arr[i + 1]];
		}

		//sets parameter mode of opcode parameter otwo
		if(paramModeTwo === 1) {
			//immediate mode
			opcodeParamTwo = arr[i + 2];
		} else if(paramModeTwo === 2) {
			//relative mode
			opcodeParamTwo = arr[relativeBase + arr[i + 2]];
		} else {
			//position mode
			opcodeParamTwo = arr[arr[i + 2]];
		}
		...
		switch(opcodeAddress) {
			...
			case 1:
				...
				if(paramModeThree === 2) {
					arr[relativeBase + arr[i + 3]] = opcodeParamOne + opcodeParamTwo;
				} else {
					arr[arr[i + 3]] = opcodeParamOne + opcodeParamTwo;
				}
				i = i + 4; //to next opcode
				break;
			case 2:
			...
			case 5:
				//address 5 might change the instruction pointer to another location in the code
				if(opcodeParamOne !== 0) {
					i = opcodeParamTwo;
				} else {
					i = i + 3
				...
				break;
			case 6:
				//address 6 might change the instruction pointer to another location in the code
				if(opcodeParamOne === 0) {
					i = opcodeParamTwo;
				} else {
					i = i + 3
				...
				break;
			case 7:
				//changes a value in the program to 1 or 0 depending on its parameters values
				if(paramModeThree === 2) {
					if(opcodeParamOne < opcodeParamTwo) {
			...
			case 99:
				...
				i = arr.length(); //exits the intcode program
				break;
			default:
...
console.log(intcodeProgram(puzzleInput,2));

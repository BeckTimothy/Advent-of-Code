...
function bruteCircuit(arr) {
	...
	return recursiveCircuit(arr, thruster);
}

function recursiveCircuit(arr, thrust) {
	let thruster = thrust;
	let newThruster = 0;
	for(a = 0; a <= 4; a++) {
		for(b = 0; b <= 4; b++) {
			for(c = 0; c <= 4; c++) {
				for(d = 0; d <= 4; d++) {
					for(e = 0; e <= 4; e++) {
						...
						//console.log(`Thruster output: ${thruster} with Sequence: a = ${a}, b = ${b}, c = ${c}, d = ${d}, e = ${e}`);
						if(ampCircuitTwo(arr, a+5, b+5, c+5, d+5, e+5, thruster) > thruster) {
							newThruster = ampCircuitTwo(arr, a+5, b+5, c+5, d+5, e+5, thruster);
						}
					}
				}
			}
	...
	return recursiveCircuit(arr, newThruster);
}
function ampCircuitTwo(arr, a, b, c, d, e, thrust) {
	let ins1 = arr;
	let ins2 = arr;
	let ins3 = arr;
	let ins4 = arr;
	...
	const ampA = intcode(ins1, a, thrust);

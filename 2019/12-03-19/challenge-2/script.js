/**
 * This is challenge 2 of 2019-12-03 of Advent of Code's 25-day challenge
 ...
 * @returns int describing the manhattan distance from the center point which two wires cross
 * @author Timothy Beck <Dev@TimothyBeck.com>
 */

//turn the wires into array where value 0 is x-axis and value 1 is y axis
//loop through the arrays one wire at a time to find all intersecting points
//store the smallest Math.abs(arr[1]) + Math.abs(arr[2]) intersection's manhattan value, and return it.

function manhattanCalc(wireOne, wireTwo) {
	...
	let wireOneDistance;
	let wireTwoDistance;
	let shortestombinedDistance = 9999999;
	let closestIntersectionDistance = 9999999999;
	//loop through first wire to create an array of positions
	for(i = 0; i < wireOne.length; i++) {
		let direction = wireOne[i].charAt(0);
		let positionsMoved = Number(wireOne[i].substr(1));
		wireBuilder(positionsMoved, wireOnePos, direction, wireOnePos, wireTwoPos, wireOneArr);
	}
	//loop through second wire to create an array of positions
	for(j = 0; j < wireTwo.length; j++) {
		let wireTwoDirection = wireTwo[j].charAt(0);
		let wireTwoPositionsMoved = Number(wireTwo[j].substr(1));
		wireBuilder(wireTwoPositionsMoved, wireTwoPos, wireTwoDirection, wireOnePos, wireTwoPos, wireTwoArr, wireOneDistance);
	}
	//loop through both arrays of positions to find matching positions
	for(i=0; i<wireOneArr.length; i++){
		for(j=0; j<wireTwoArr.length; j++){
			wireOneDistance = i + 1;
			wireTwoDistance = j + 1;
			if(wireOneArr[i][0] === wireTwoArr[j][0] && wireOneArr[i][1] === wireTwoArr[j][1] && closestIntersectionDistance > Math.abs(wireOneArr[i][0]) + Math.abs(wireOneArr[i][1])) {
				//if so, store the manhattan distance in variable
				if(wireTwoDistance + wireOneDistance < shortestombinedDistance){shortestombinedDistance = wireOneDistance + wireTwoDistance;}
				closestIntersectionDistance = Math.abs(wireOneArr[i][0]) + Math.abs(wireOneArr[i][1]);
			}
		}
	}
	console.log(wireOneArr);
	console.log(wireTwoArr);
	//return closestIntersectionDistance;
	return shortestombinedDistance;
}

function wireBuilder(positionsMoved, startingPoint, direction, wireOnePos, wireTwoPos, wireArr, wireDistance) {
	for(k = Math.abs(positionsMoved); k > 0; k--) {
		if(direction === 'R'){
			wireArr.push([startingPoint[0] + 1, startingPoint[1]]);
			...
			startingPoint[0] = startingPoint[0] - 1;
			wireDistance++;
			//wireArr.push(startingPoint);
		}else if(direction === 'U'){
			wireArr.push([startingPoint[0], startingPoint[1] + 1]);

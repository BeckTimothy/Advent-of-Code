/**
 * This is challenge 2 of 2019-12-06 of Advent of Code's 25-day challenge
...
function orbitCalc(arr) {
	...
	//create an array of arrays that contains all orbits
	...
	//count orbits and create santa/you arrays containting santa's and your distance from center
	let totalOrbits = 0;
	let santaPath = [];
	let youPath = [];
	for(i=0; i<paths.length; i++) {
		...
		if(paths[i][0].slice(-3, paths[i][0].length) === 'SAN'){
			santaPath = paths[i];
		}
		if(paths[i][0].slice(-3, paths[i][0].length) === 'YOU'){
			youPath = paths[i];
		}
	}
	let removed = [];
	//loop through both paths and remove all non unique orbits, answer will equal length - 2
	for(i=0; i<santaPath.length; i++){
		for(j=0; j<youPath.length; j++) {
			if (santaPath[i] === youPath[j]) {
				removed = santaPath.splice(i, i);
				removed = youPath.splice(j, j);
			}
		}
	}
	console.log(santaPath.concat(youPath).length - 2);
}
